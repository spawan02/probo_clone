...
const doOrder = (data) => __awaiter(void 0, void 0, void 0, function* () {
    const { userId, quantity, actualPrice, stockType, type, stockSymbol } = JSON.parse(data);
    const price = actualPrice;
    switch (type) {
        case "buyOrderOption":
            if (!(userId in order_1.INR_BALANCES)) {
                return ({ error: true, msg: "user doesn't exist" });
            }
            else {
                console.log(order_1.STOCK_BALANCES[userId][stockSymbol]);
                // if(!(STOCK_BALANCES[userId][stockSymbol])){
                //     return({error: true, msg: "stock doesn't exist"})
                // }    
                if (!order_1.STOCK_BALANCES[userId][stockSymbol]) {
                ...
                let UserStockBalance = order_1.INR_BALANCES[userId].balance;
                ...
                order_1.INR_BALANCES[userId]["balance"] -= totalRequiredPrice;
                order_1.INR_BALANCES[userId]['locked'] += totalRequiredPrice;
                //@ts-ignore
                const stockOrderBook = order_1.ORDERBOOK[stockSymbol];
                const reverse = (type) => {
                    if (type === "yes") {
                        return "no";
                    }
                ...
                // }
                //@ts-ignore
                let stockOrderBookPrice = order_1.ORDERBOOK[stockSymbol][stockType][price];
                ...
                if (!(stockOrderBook[stockType][price])) {
                    ...
                    const reverseOrderBook = order_1.ORDERBOOK[stockSymbol][reverseStockType];
                    ...
                    if (reverseOrderBook[reversePrice]) {
                        ...
                        if (existingReverseQuantity < quantity) {
                            order_1.ORDERBOOK[stockSymbol][reverseStockType] = Object.assign(Object.assign({}, reverseOrderBook), { [reversePrice]: {
                            ...
                            //@ts-ignore
                            order_1.STOCK_BALANCES[userId][stockSymbol][stockType]["quantity"] = existingReverseQuantity;
                            // INR_BALANCES[userId]["balance"] -= ((100*price)*existingReverseQuantity)
                            order_1.INR_BALANCES[userId].locked -= existingReverseQuantity * 100 * price;
                            ...
                            return ({ error: false, msg: order_1.ORDERBOOK[stockSymbol] });
                        }
                        //if the available order is equal to quantity the user stock balance and INR balance is updated 
                            ...
                            for (const user in ordersOrderBook) {
                                ...
                                tempQuantity -= toTake;
                                //@ts-ignore
                                order_1.STOCK_BALANCES[userId][stockSymbol][stockType]["quantity"] = quantity;
                            ...
                            let balance = order_1.INR_BALANCES[userId]["balance"];
                            balance -= ((Number(reversePrice) * 100) * quantity);
                            order_1.INR_BALANCES[userId]["balance"] = balance;
                            order_1.INR_BALANCES[userId]["locked"] = 0;
                            return ({ error: false, msg: order_1.ORDERBOOK[stockSymbol] });
                        }
                    }
                    else {
                    ...
                    if (stockOrderBookPrice["total"] >= quantity) {
                        ...
                        for (const user in ordersOrderBook) {
                            //@ts-ignore
                            const available = order_1.ORDERBOOK[stockSymbol][stockType][price].orders[user].quantity;
                            const toTake = Math.min(available, quantity);
                            //@ts-ignore
                            order_1.ORDERBOOK[stockSymbol][stockType][price].orders[user].quantity -= toTake;
                            //@ts-ignore
                            order_1.ORDERBOOK[stockSymbol][stockType][price].total -= toTake;
                            tempQuantity -= toTake;
                            //updating the user orderbook balance 
                            if (ordersOrderBook[user].type === "sell") {
                                //@ts-ignore
                                order_1.STOCK_BALANCES[user][stockType][stockType][locked] -= toTake;
                                order_1.INR_BALANCES[user].balance += toTake * 100 * price;
                                order_1.INR_BALANCES[userId].locked -= toTake * 100 * price;
                            }
                        ...
                        //@ts-ignore 
                        order_1.STOCK_BALANCES[userId][stockSymbol][stockType]["quantity"] = stockOrderBookPrice["total"];
                        stockQuantityBalance += stockOrderBookPrice["total"];
                        stockOrderBookPrice["total"] = 0;
                        stockOrderBookPrice["orders"] = {};
                        const reverseStockType = reverse(stockType);
                        //when there are no 'yes' orders to be fulfilled at user price
                        order_1.ORDERBOOK[stockSymbol][reverseStockType] = Object.assign(Object.assign({}, order_1.ORDERBOOK[stockSymbol][reverseStockType]), { [reversePrice]: {
        ...
        case "sellOrderOption":
            //@ts-ignore
            if (!(userId in order_1.STOCK_BALANCES) || !(order_1.STOCK_BALANCES[userId][stockSymbol])) {
                ...
                if (!(stockQuantity >= quantity)) {
                    ...
                    priceLevel = {
                        total: 0,
                        orders: Object.assign(Object.assign({}, priceLevel.orders), { [userId]: {
                                'quantity': quantity,
                                type: 'sell'
                            } })
                    };
                    //@ts-ignore
                    let order = order_1.ORDERBOOK[stockSymbol][stockType];
                    order = Object.assign(Object.assign({}, order), { [price]: priceLevel });
                    //@ts-ignore
                    order_1.ORDERBOOK[stockSymbol][stockType] = order;
                ...
                //@ts-ignore
                const pubMessage = order_1.ORDERBOOK[stockSymbol];
                // const responseObj = {
                //     stockSymbol,
